...
class Node(BaseSchema):
    ...
    parent: Optional["Node"] = Field(default=None, exclude=True, description="None for root")
    ...
    def add_child(
        ...
        resolved_title = title if title is not None else self.get_title_for_level(next_level)
        ...
        child = Node(
            # BaseSchema fields
            title=resolved_title or "",          # title is required by BaseSchema; fallback to empty string
